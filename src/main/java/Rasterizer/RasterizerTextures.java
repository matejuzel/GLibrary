...
public class RasterizerTextures extends RasterizerAbstract {
    ...
    public double scanEdge(
        ...
        for (int line = lineL; line < lineR; line++) {
            ...
            double[] atrs_L_k = {
                (atrs_L0[0] + kL * atrs_dL[0]) * zL_k,
                (atrs_L0[1] + kL * atrs_dL[1]) * zL_k,
                (atrs_L0[2] + kL * atrs_dL[2]) * zL_k,
                (atrs_L0[3] + kL * atrs_dL[3]) * zL_k,
                (atrs_L0[4] + kL * atrs_dL[4]) * zL_k,
            };
            double[] atrs_R_k = {
                (atrs_R0[0] + kR * atrs_dR[0]) * zR_k,
                (atrs_R0[1] + kR * atrs_dR[1]) * zR_k,
                (atrs_R0[2] + kR * atrs_dR[2]) * zR_k,
                (atrs_R0[3] + kR * atrs_dR[3]) * zR_k,
                (atrs_R0[4] + kR * atrs_dR[4]) * zR_k,
            };
    ...
    public void scanLine(int x0, int x1, int y, double z0, double z1, double[] atrs0, double[] atrs1) {
        ...
        for (int i=0; i<dx + 1; i++) {
            ...
            if (depthBuffer.write(x, y, z_k)) {
                ...
                //frameBuffer.putPixel(x, y, (int)atrs_k[2], (int)atrs_k[3], (int)atrs_k[4]);
            }
            x++;
            k += dxInv;
        }
